package school.faang.user_service.filter;

import org.springframework.stereotype.Component;
import school.faang.user_service.dto.event.EventFilterDto;
import school.faang.user_service.entity.event.Event;

import java.util.stream.Stream;

@Component
public class EventTitleContainsFilter implements EventFilter {
    @Override
    public boolean isApplicable(EventFilterDto eventFilterDto) {
        return eventFilterDto.titleContains() != null
                && !eventFilterDto.titleContains().isBlank();
    }

    @Override
    public Stream<Event> apply(Stream<Event> events, EventFilterDto eventFilterDto) {
        String searchTerm = eventFilterDto.titleContains().toLowerCase();
        return events
                .filter(event -> event.getTitle() != null
                        && event.getTitle().toLowerCase().contains(searchTerm));
    }
}
